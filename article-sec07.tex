...
\end{itemize}
In practice, the ``topological descriptors'' can be drawn from standard tools in topological data analysis (TDA), such as persistent homology and associated barcodes~\cite{carlsson2009_topology_data}, while the response-based features can be implemented using physical reservoir computing platforms in which a photonic or electronic reservoir acts as a high-dimensional nonlinear feature map~\cite{tanaka2019_physical_reservoir,vandersande2017_photonic_reservoir}. This anchors the abstract notion of algorithmic similarity in a toolbox that is already used for analysing complex spatiotemporal patterns.
