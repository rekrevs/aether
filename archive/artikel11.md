...
\[
\boxed{\;\alpha\equiv 1\ \text{(exakt)}\;}
\]
överallt. Metrisk respons är därmed ljuskonstroget; all upplevd FTL kommer enbart från \(S\)-lokalitet.
...
\tag{4.4}
\]
Heaviside‑faktorn säkerställer uttrycklig substrat‑retardering redan på kärnnivå.
...
**Explicit exempel (lokal, gauge- & diffeo‑invariant \(O_S\) med \(\Delta>4\)).** Välj en lokalt definierad fönsterfunktion \(w_\ell\) med kompakt stöd och låt
\[
...
(\nabla\phi)^m(\nabla\nabla\phi)^n(R)^k,
\]
så att \(\Delta>4\) garanterar irrelevans i RG‑mening samtidigt som operatorn är strikt lokal i \(M\). Signaturerna i §7 (FFT, persistent homologi, Wasserstein) implementeras operativt genom att låta koefficienterna \(c_{mnk}\) bero på lokalt extraherade statistik‑/topologimoment av fälten inom \(w_\ell\).

**Naturlighet:** Varför blir \(O_S\neq0\)? En möjlig mekanism är svag, spontan symmetribrytning i substratet som inducerar små men icke‑noll lokala invariantkombinationer nära kritikalitet.
...
**Förregistrering:** Distansstegen för‑registreras som **separat, blind pilot** (egen OSF‑DOI) innan huvudförsöken E1/E2/E3. Blinda "nivåetiketter" hålls förseglade till analysfasen.
...
\tag{9.1}
\]
Heaviside‑"gating" gör substrat‑kausaliteten explicit även i operatornorm‑gränsen.
...
**Formellt bevis (Appendix C):** Duhamel‑expansion av tidsutvecklingsoperatorn, uppdelning i lokal (M) och icke‑lokal (S) del, grafnorm‑estimat med sparsamma villkor, samt explicit substrat‑retardering via \(\Theta(t-d_\sigma/c_S)\); linjärterm saturerar vid \(t\sim\hbar/\eta\).
...
För \(\lambda_\sigma \sim 1\,\mu{\rm m}\), \(\lambda_C \sim 10^{-12}\,{\rm m}\) (Compton) ⇒ \(\lambda_\sigma/\lambda_C \sim 10^6\), så \(\varepsilon\cdot \mathcal Q \lesssim 10^{-24}\).
...
### 11.3. SME‑parametrisering (för rapportering och jämförelse)

I fotonsektorn uttrycks anisotropier lämpligen i minimala SME‑koefficienter \(\tilde\kappa_{e-}^{JK},\,\tilde\kappa_{o+}^{JK},\,\kappa_{\rm tr}\). Vår modell ger effektivt
\[
\tilde\kappa_{e-}^{JK}\ \sim\ \varepsilon_\gamma\,\mathcal Q_\gamma\,
\Big(\frac{\lambda_\sigma}{L_{\rm exp}}\Big)\,\Xi^{JK},
\]
med \(\Xi^{JK}\) en geometri‑faktor (\(\mathcal O(1)\)). Rapportera därför \(\hat A_{\rm sid}\) parallellt med estimat på \(|\tilde\kappa_{e-}^{JK}|\) för att möjliggöra direkt jämförelse med resonator‑ och atomklock‑studier.
...
**Statistisk metodruta:** Vi för‑registrerar beslutsregler och justerar för **multipla jämförelser** (t.ex. **Holm–Bonferroni** eller FDR‑korrigering) över distansnivåer (6 nivåer i §7.3), kontrollprotokoll (match/mismatch, on/off) och experiment (E1–E3) för att kontrollera family‑wise error rate vid \(\alpha=0.05\). Distansstege‑piloten (se §7.3) publiceras med egen DOI och används som förhandskalibrering.
...
1a. **Blind pilot (distansstege):** Kör en separat, blint märkt nivå‑serie (N=6) och fryser \((\lambda_\sigma,\ell_0)\) innan huvudkörningen.
...
\tag{C.12}
\]
Detta ger den använda formen i §9, och motiverar "mjuk kon" med explicit substrat‑retardering, exponentiell \(d_\sigma\)‑dämpning och icke‑saturerande S‑bidrag.
...
| \(\alpha\) | Grav‑koppling S‑energi | 1 | **\(1\)** (exakt; Bianchi‑identitet) |
...
## Appendix G: Effektiv bilokal kärna på \(M\) – härledning och positivitet

Utgår från (3.2) och push‑forward (§3.3). Definiera
\[
...
\tag{G.1}
\]
**Retardering:** Heaviside‑faktorn implementerar substrat‑kausalitet.  
**Positivitet (Schoenberg‑typ):** För testfunktion \(\varphi\) gäller
\[
...
\ge 0,
\]
med \(\psi(\sigma)=\!\int d^4x\, f_\ell(x-\pi\sigma)\,\varphi(x)\). Alltså är \(\mathcal{K}_{\rm eff}\) positiv semidefinit om \(\mathbb{K}\) är det.

## Appendix H: Ward‑identitet för (3.7) – noll nettoimpuls

Betrakta \(S_{int}^{\rm eff}=\varepsilon\!\iint d^4x\,d^4x'\,O_S(x)\,\mathcal{K}_{\rm eff}(x,x')\,O_S(x')\) med \(\mathcal{K}_{\rm eff}(x,x')=\mathcal{K}_{\rm eff}(x',x)\) och translational‑invarians. Infinitesimal translation \(x^\mu\!\to\! x^\mu+a^\mu\) ger
\[
\delta S_{int}^{\rm eff}=a_\nu\!\int\! d^4x\,\partial_\mu \big(T^{\mu\nu}_{\rm int}\big).
\]
Variation visar \(\partial_\mu T^{\mu i}_{\rm int} = - J^i_\sigma\) och tillsammans med (3.4)–(3.5) fås
\(\partial_\mu T^{\mu i}_{\rm tot}=0\). Integration över rummet ger
\[
\frac{d}{dt}\,P^i_{\rm tot}=\frac{d}{dt}\!\int d^3x\, T^{0i}_{\rm tot}=0,
\]
vilket är (3.7): ingen reaktionsfri drivning även under tidsberoende pump. Randtermer elimineras av kompakt stöd hos \(f_\ell\) och symmetrin \(\mathcal{K}_{\rm eff}(x,x')=\mathcal{K}_{\rm eff}(x',x)\).

## Appendix I: SME‑koppling – rapporteringsformat

För jämförelse med litteraturen rapporteras foton‑anisotropier i minimala SME‑koefficienter. I vår modell leder preferensriktningen \(\xi^\mu\) till
\[
...
\Big(\frac{\lambda_\sigma}{L_{\rm exp}}\Big)\,\Xi^{JK}.
\]
Vid rapportering av \(\hat A_{\rm sid}\) i E2 bör uppskattat \(|\tilde\kappa_{e-}^{JK}|\) anges med felstaplar och kopplas till (11.1).
