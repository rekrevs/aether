...
Vi formulerar en konsekvent, falsifierbar hypotes där observerbar relativistisk fysik är en emergent låg-energi-beskrivning av ett diskret substrat med global uppdateringsordning. Utöver rum-tidsnärhet i den emergenta mångfalden (M) antar vi **strukturnärhet** i ett mönsterrum (S) som mäter algoritmisk likhet mellan lokala tillstånd. En **substrat-lokal** koppling i (S) — **eterresonans** — kan då transportera energi och information mellan isomorfa delstrukturer och projiceras som FTL i (M). Vi ger: (i) **action-nivå** med väldefinierat (T^{μν}_S), (ii) **kovariant** bevarandelag och lokalisering av (S)-flöden som källor i (M), (iii) **momentum-neutralitet**, (iv) en **selektionsoperator** (O_S) som gör effekten osynlig i standardsektorn, (v) en **operationaliserad** (d_σ)-metrik och dimensionellt korrekt kopplingslag, (vi) en **lokal implementering** av (S)-närhet via LSH, (vii) en **mjuk Lieb-Robinson-bound**, (viii) ett **formellt kausalitetsargument**, (ix) en **termodynamisk resursolikhet** med bitrates-bound, (x) en **anisotropi-budget** och (xi) stringenta, **no-loophole** experiment (E1–E3) med kartlagd parameterinferens.
...
* **(S)**: mönsterrum vars punkter är lokala datastrukturer; avstånd (d_σ) mäter algoritmisk (isomorfi-)likhet.
...
vilket förhindrar **reaktionsfri drivning** (energi kan överföras utan nettoimpuls i (M)). Detta är konsistent med (3.4) integrerad över en sluten volym.
...
* **Val:** Vi antar (α ≈ 10^{-10}) → S-energi graviterar nästan inte → **ingen mätbar FTL gravitationssignal**, förenligt med GR-tester. Detta kräver en förklaring från underliggande substratdynamik (t.ex. induced-gravity-mekanism där S-sektor kopplar svagare till metrisk emergensen).
...
* (K(e) = exp[-d_σ(e)/λ_σ]) är **likhetskärnan** (dimensionslös),
* (Q(e,t) ∈ [0,1]) är **koherensfaktor** (faslåsning, kritikalitet, dimensionslös),
* (𝓚(e,t) = P_{pump}(e) · τ / (ħω₀)) är **dimensionslös kostnadsfaktor** (normerad pumpeffekt),
...
Ett nollresultat ger direkt övre gräns:
...
där (I_{null}) är detektionsgräns för informationsöverföring. Detta översätter nollutfall direkt till parameterbounds.
...
**Sats 9.1 (skiss).** Om (η ≪ v) (S-flöden svaga), (E_S) är **sparse** (grad ≪ N), och (W_σ) är retarderad i (τ), då finns en **mjuk kon**:
...
**Intuition:** S-kanter ger sällsynta, långräckviddshopp men sparsamhet och svag koppling gör dem till små perturbationer. Klusterdekomposition bryts till (O(η/v)), förenligt med experiment om (η/v < 10^{-10}). Detta kvantifierar mikrokausalitetsbrott.
...
**Sats 10.1 (Kausal monotoni).** Under villkoren (i) all resonansdynamik är retarderad i substratordningen (T), (ii) varje resonanssteg kräver (𝓚(e) ≥ 0), och (iii) ε är ändlig, finns inga slutna kausala loopar i (M×S).
...
**Null-bound:** Om (BER ≥ 0.49) för alla konfigurationer → (ε λ_σ Q < 10^{-12}) (dimensionslöst · m).
...
* (ω₀ ~ 10 GHz), (ħω₀ ~ 10^{-23} J),
* (K ~ 0.5) (matching god), (Q ~ 10^{-5}) (nära tröskel), (𝓚 ~ 100) (pump), (Δ̃Φ ~ 1),
* (ε ~ 10^{-15}).
...
7. **Momentum-test:** Verifiera (3.7) experimentellt (inga spår av reaktionsfri kraft).
...
**Inferens:** Log-linjär fit ger (λ_σ); nollresultat → (λ_σ)-bound. Mappning: nollresultat → (ε Q < 10^{-15}).
...
* **α-värde ad hoc:** Vi antar (α ~ 10^{-10}) för att undvika gravitationell FTL-signal, men detta kräver förklaring från substratdynamik (t.ex. induced-gravity-mekanism, Sakharov-typ).
...
Genom att härleda ramverket från en **variationsprincip** (säkerställer konsistens med GR via α-koppling och väldefinierat (T^{μν}_S)), introducera **momentum-neutralitet** (förhindrar reaktionsfri drivning), ge **lokal LSH-implementation** (gör S-närhet implementerbar), presentera en **modifierad Lieb-Robinson-bound** (kvantifierar klusterdekompositionsbrott), och ge ett **formellt kausalitetsbevis** (garanterar inga loopar), har vi adresserat de kritiska konsistensinvändningarna.

**Termodynamiska resursgränser** (bitrates-bound) och **numeriska prediktioner** gör ramverket **falsifierbart**: nollresultat i E1/E2/E3 översätts direkt till övre gränser på (ε, λ_σ, Q, ω_0). Positiva resultat skulle öppna för en ny klass av icke-lokala teknologier.
...
3. **Lokalisering av S-flöden** (§3.3) som källor i M,
4. **Momentum-neutralitet** (§3.4) som förhindrar reaktionsfri drivning,
5. **Lokal LSH-implementation** (§4) av S-närhet,
6. **Selektionsoperator** (§5) som förklarar frånvaro i standardexperiment,
7. **Dimensionellt korrekt kopplingslag** (§6) med full enhetsanalys,
8. **Operationaliserad d_σ-metrik** (§7) och distansstege,
9. **Termodynamiska resursgränser** (§8) med bitrates-bound,
10. **Modifierad Lieb-Robinson-bound** (§9) som kvantifierar mikrokausalitetsbrott,
11. **Formellt kausalitetsbevis** (§10) via kategori-teori,
12. **Anisotropi-budget** (§11) med kvantitativa bounds,
13. **Numeriska prediktioner** (§12) med explicita mål och null-bounds,
14. **No-loophole experiment** (§13) med parameterinferens.
...
* **𝓚 (kalligrafisk K):** dimensionslös kostnadsfaktor (= P_{pump}·τ / ħω₀).
...
Summa ger (3.5) med (1-α)-korrektion om S graviterar svagare.
