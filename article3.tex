...
\textbf{Power form (dimension fixing):} the flow is written directly as power
\begin{equation}
...
\textbf{Degeneracy dilution.} For a periodic system with $(N)$ equivalent matches, $(J_\sigma \to J_\sigma / N)$. For a cubic lattice with volume V and lattice constant a: $(N \sim V/a^3 \to \infty)$, explaining absence in homogeneous systems.
...
\begin{equation}
E^2/c^2 - p^2 = m^2 + \delta(E, \hat{p}\cdot\hat{\xi}),
\end{equation}

where

\begin{equation}
\delta \sim \varepsilon (\lambda_\sigma / \lambda_C) \mathcal{Q} \cdot (E / m c^2) \cdot (\hat{p}\cdot\hat{\xi})^2.
\end{equation}
...
\item No robust effects in homogeneous crystals $(N \gg 1 \to J_\sigma \to 0)$.
...
where $(J_\sigma \sim \varepsilon \hbar\omega_0 K Q \mathcal{K} \tilde{\Delta\Phi})$.
...
\begin{itemize}
\item $P_{\rm pump}=1\,\mu\mathrm{W}$, $K\simeq0.5$, $\tilde{\Delta\Phi}\simeq1$,
\item $\varepsilon=10^{-15}$, $\mathcal Q=10^{-8}$ (compatible with (11.1)),
\item[] $\to J_\sigma\approx 5\times10^{-30}\,\mathrm{W}$ $\Rightarrow \Delta E(10^3\mathrm{~s})\approx 5\times10^{-27}\,\mathrm{J}$.
\end{itemize}
...
\textbf{Important:} The prediction here concerns the \textbf{matter sector} $(\varepsilon_{\rm mat}\mathcal Q_{\rm mat})$. Bounds in §11.1 constrain $\varepsilon_\gamma\mathcal Q_\gamma$ (optics) and are therefore not directly binding on (12.3a); see Appendix F (sector separation).

\textbf{Cross-reference:} Report $\hat{A}_{\rm sid}$ and compare against (11.2) and (12.3a–b); specify which measurement track (optical vs. material) is used in the rig.
