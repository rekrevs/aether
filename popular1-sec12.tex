...
$BER_{min} = (1 - \betaBER)/2 \approx 0.25$ (for $\betaBER \sim 0.5$)
...
\begin{equation}
  \Delta E_A + \Delta E_B \;=\; \Psig\,\Delta t \,,
  \tag{12.2}
\end{equation}

where the substrate power flow is:

$\Psig \sim \varepsilon \hbar\omega_0 \Ksig \mathcal{Q} \Krate \tilde{\Delta\Phi}$
...
\begin{itemize}
\item $P_{pump} = 1$ $\mu$W
\item $\Ksig \approx 0.5$ (50\% structural match)
...
\begin{equation}
  A_{\rm sid}^{(\mathrm{mat})} \;\simeq\; \frac{\epsmat}{\epsgam} \cdot \frac{\lsig}{L_{\rm exp}} \cdot \frac{\Qmat}{\Qgam} \cdot \Xi .
  \tag{12.4}
\end{equation}

\textbf{Important distinction:}\\
The matter sector amplitude (12.4) uses the ratio $\epsmat/\epsgam$ (matter to photon coupling) and $\Qmat/\Qgam$ (matter to photon quality factors). The \textbf{optical sector} bounds from Michelson-Morley experiments constrain $\epsgam \cdot \Qgam$ but don't directly bind matter-sector measurements.

The bounds in \S11 (Eq. 11.4) constrain $\epsgam \cdot \Qgam$ (optics). They don't directly bind (12.4) in the matter sector - see discussion of sector separation.
...
$\epsmat \cdot \Qmat < 10^{-20}$ (for given $L_{exp}$ and $\lsig$ - report both)
