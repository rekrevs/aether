...
\begin{equation}
  ...
  \tag{5.2}
\end{equation}
Here $\Lambda$ is a fixed UV scale of the visible sector. Choosing $\Delta>4$ means that, in ordinary effective field theory, the dimensionless coupling
associated with $\mathcal{O}$ would decrease in the infrared, and high-energy
experiments would normally give the strongest constraints. In the present framework we \emph{do not} rely on such RG running to hide the effect. Instead, what suppresses visible-sector signatures is the state-dependence of $\langle O_S\rangle$ itself, as explained next.
