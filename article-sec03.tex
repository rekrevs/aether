...
\frac{J_S(x,\sigma)\,\Kern(\sigma,\sigma')\,O_S(x,\sigma')}{\LamUV^{4}},
\end{split}
\tag{3.1}
\end{equation}
...
\item The last term encodes the substrate-local coupling in $(\Sspace)$ via a current $J_S$, a kernel $\Kern$ on $(\Sspace,\Dsig)$, and a selection operator $O_S$.
\end{itemize}

\subsection{Aether / Khronon Sector}

We choose a simple baseline aether/khronon sector that is compatible with existing constraints. A conservative option is to take a unit timelike vector field $u^\mu$ enforced by a Lagrange multiplier:
\begin{equation}
...
\tag{3.1C}
\end{equation}
so that gravitational-wave speed $c_T$ matches $c$ to high precision and the preferred frame effects are within current bounds. For the bulk of this paper, option (A) will suffice; (B) simply provides a conservative embedding into the broader Einstein--æther literature and leaves room to maneuver if one later wishes to let $u^\mu$ carry weak, long-wavelength structure.
