...
* **Val och motivering för α:** Vi antar (α ≈ 10^{-10}) → S-energi graviterar nästan inte → **ingen mätbar FTL gravitationssignal**, förenligt med GR-tester.

**Antagande om α-mekanism (Assumption Box):**

> Vi antar en **Sakharov-lik induced-gravity-mekanism** där S-sektorns bidrag till metriska fluktuationer renormaliseras ned till (α ∼ 10^{-10}) i låg-energi-gränsen. Detta kan uppstå om graviton-propagatorn får dominanta bidrag från synliga fält medan S-sektorns koppling till metriken är undertryckt genom en loop-faktor eller svag mixing. Explicit härledning från en mikro-substratmodell lämnas till framtida arbete, men kravet (α ≪ 1) är **nödvändigt** för konsistens med Eötvös-bounds och för att undvika FTL-gravitationssignaler.
...
**Lemma 9.1 (Mjuk kon med explicita villkor).** Antag:
* (i) Maximal grad: varje nod (x) har högst (g) långräckviddiga S-kanter, (g ≪ N),
* (ii) Normbound: för varje (x), (Σ_{e ∋ x} |J_σ(e)| ≤ η),
* (iii) Retardering: (W_σ) är kausal i (τ),
* (iv) Tidsfönster: (t ≤ ħ / η),
* (v) (η ≪ ħ v).

Då finns en **mjuk kon**:

\[
||[A(x,t), B(y,0)]|| \leq C \left( e^{-\kappa(|x-y| - v_{eff} t)} + D \, \frac{\eta t}{\hbar} \right),
\tag{9.1}
\]

där (v_{eff} ≈ v + δv), (δv ∝ η), och konstanterna (C, D, κ) beror på operator-normer och grafstruktur.

**Intuition:** S-kanter ger sällsynta, långräckviddshopp men sparsamhet (villkor i–ii) och svag koppling (villkor v) gör dem till små perturbationer. Klusterdekomposition bryts till (O(η/v)), förenligt med experiment om (η/v < 10^{-10}). Detta kvantifierar mikrokausalitetsbrott.

**Tidsfönster-notering:** Andra termen i (9.1), (D·η t/ħ), växer linjärt i tiden och dominerar endast för (t ≳ ħ/η). För typiska experimentella tidsskalor (t ~ 1–10³ s) och (η ~ 10^{-30}–10^{-28} J) ger detta (ħ/η ~ 10^{-5}–10^{-3} s), vilket innebär att S-bidraget kan bli märkbart på millisekund–sekund-skala men förblir litet (\ll 1) så länge (η ≪ ħv).

**Formellt bevis (Appendix C):** Duhamel-expansion av tidsutvecklingsoperatorn, uppdelning i lokal (M) och icke-lokal (S) del, grafnorm-estimat med sparsamma villkor.
