...
\item \textbf{Naturalness of small couplings.} If $\varepsilon$ and typical $\mathcal Q$ values are very small (as suggested by photon-sector bounds), one may ask why the exact values are not zero. The present framework does not yet identify a symmetry principle that both permits and suppresses aether resonance, so the hierarchy between ``allowed'' and ``observably tiny'' couplings is currently an assumption rather than a derived result.
\end{itemize}
